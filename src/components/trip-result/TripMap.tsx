      ...
      >
        ...
        {stays.map((stay, i) => (
          stay.coordinates?.lat && stay.coordinates?.lng && (
            <Marker 
              key={`stay-${i}`} 
              latitude={Number(stay.coordinates.lat)} 
              longitude={Number(stay.coordinates.lng)}
              onClick={(e) => {
                e.originalEvent.stopPropagation();
                handleMarkerClick({ lat: Number(stay.coordinates.lat), lng: Number(stay.coordinates.lng) });
              }}
            >
              <div className="group relative">
                ...
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-slate-900 text-white text-[10px] rounded opacity-0 group-hover:opacity-100 whitespace-nowrap pointer-events-none transition-opacity">
                  <img src={stayIcon} className="object-contain" alt="" /> {stay.name}
                </div>
              </div>
            </Marker>
          )
